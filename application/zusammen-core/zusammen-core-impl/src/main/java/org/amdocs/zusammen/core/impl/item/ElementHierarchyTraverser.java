...
public class ElementHierarchyTraverser {
  ...
  public void traverse(SessionContext context, ElementContext elementContext,
                       Space space, CoreElement element, ElementVisitor visitor) {
    visitor.visit(context, elementContext, space, element);
    element.getSubElements().forEach(subElement ->
        traverse(context, elementContext, space, subElement, visitor)
    );
  }
}
